...
ax.plot(np.arange(1,n_it+1),avg_cost_10_perf[3],"--o",label = "T=4",linewidth = 1)
...
Ts_plot = [1,2,3,4]
ax.legend(["T={}".format(i) for i in Ts_plot],loc = (0.75,0.45))
#x_tick_labels = ["T={}".format(i) for i in Ts_plot]
...
fin_cost_perf_list = avg_cost_10_perf[:,n_ep-1]
fin_cost_no_perf_list = avg_cost_no_perf[:,n_ep-1]
...
"""
print("--- Results SafeMPC, T=2, H in {5,10,15} ---")
print("H=5")
a_safe_2_perf_5 = np.load("{}/{}/{}/results_episode.npy".format(results_path,sub_folder_path,savedir_safempc("2","5","1","2_0"))).item()
print(_get_failure_ratio(a_safe_2_perf_5["safety_failure_all"]))
avg_cost_safempc_h5, _ = cost_per_episode_scenario(np.array(a_safe_2_perf_5["cc_all"]),np.array(a_safe_2_perf_5["safety_failure_all"]),(n_scen,n_ep,n_steps),safe_avg = True, norm = normalizer)
print(avg_cost_safempc_h5)
"""

"""
print("H=10")
a_safe_2_perf_10 = np.load("{}/{}/{}/results_episode.npy".format(results_path,sub_folder_path,savedir_safempc("2","10","1","2_0"))).item()
print(_get_failure_ratio(a_safe_2_perf_10["safety_failure_all"]))
avg_cost_safempc_h10, _ = cost_per_episode_scenario(np.array(a_safe_2_perf_10["cc_all"]),np.array(a_safe_2_perf_10["safety_failure_all"]),(n_scen,n_ep,n_steps),safe_avg = True, norm = normalizer)
print(avg_cost_safempc_h10)
"""

print("H=15")
a_safe_2_perf_15 = np.load("{}/{}/{}/results_episode.npy".format(results_path,sub_folder_path,savedir_safempc("2","15","1","2_0"))).item()
print(_get_failure_ratio(a_safe_2_perf_15["safety_failure_all"]))
avg_cost_safempc_h15, _ = cost_per_episode_scenario(np.array(a_safe_2_perf_15["cc_all"]),np.array(a_safe_2_perf_15["safety_failure_all"]),(n_scen,n_ep,n_steps),safe_avg = True, norm = normalizer)
print(avg_cost_safempc_h15)
