...
def _k_lin_rbf(x,hyp,y = None,diag_only = False):
    ...
    k_prod_rbf = _k_rbf(x_rbf,y_rbf,prod_rbf_variance,prod_rbf_lengthscale,diag_only)
...
def _k_lin_mat52(x,hyp,y=None,diag_only = False):
    ...
    k_prod_mat52 = _k_mat52(x_m52,y_m52,prod_mat52_variance,prod_mat52_lengthscale,diag_only)
...
def _get_kernel_function(kern_type,hyp):
    ...
    if kern_type == "rbf":
        return lambda x, y = None, diag_only = False : _k_rbf(x,y=y,diag_only=diag_only,**hyp)
    elif kern_type == "lin_rbf":
        return lambda x, y = None , diag_only = False: _k_lin_rbf(x,hyp,y=y,diag_only=diag_only)
    elif kern_type == "lin_mat52":
        return lambda x, y=None, diag_only = False: _k_lin_mat52(x,hyp,y=y,diag_only=diag_only)
    else:
