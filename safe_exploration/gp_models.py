...
class GaussianProcess(StateSpaceModel):
    ...
    def __init__(self,n_s_out,n_s_in,n_u,kerns,X=None,y=None,m=None, train = False, Z = None):
        """ Initialize GP Model ( possibly without training set)
    ...
    def choose_datapoints_maxvar(self,x,y,m,k = 10,min_ratio_k = 0.25,n_reopt_gp = 1):
        ...
        for i in range(m-k):
            print("size x_pool: {}".format(np.shape(x_pool)[0]) )
            print("size x_chosen: {}".format(np.shape(x_chosen)[0]) )
    ...
    def predict(self,states,actions,jacobians = False, full_cov = False):
        ...
        for i in range(self.n_s_out):
            y_mu_pred[:,None,i],y_sigm_pred[:,None,i] = self.gps[i].predict_noiseless(z_new)
    ...
    def predict_2nd_order_directional_derivative(self, state, action, hess_cov = False, full_cov = False):
        """ Compute predictive mean and variance with 2nd order derivatives 

        Parameters
        ----------

        """
        raise NotImplementedError("May need this for the gradients of the linearization (derivatives of jacobian)")
...
class CasadiSSMEvaluator(cas.Callback):
    ...
    """
    def __init__(self,ssm, linearize_mu = False, opts={}):
        """
    ...
    def get_n_in(self): return 2 #state and action
    def get_n_out(self): return 2  #mean and variance
    ...
    def get_sparsity_out(self,i):
        ...
        return [cas.Sparsity.dense(self.ssm.num_states,1),cas.Sparsity.dense(self.ssm.num_states,1),cas.Sparsity.dense(self.ssm.num_states,self.ssm.num_states+self.ssm.num_actions)][i]
    ...
    def get_jacobian(self,name,inames,onames,opts):
        ...
        class JacFun(cas.Callback):
            ...
            def get_sparsity_in(self,i):
                return [cas.Sparsity.dense(self.ssm.num_states,1),cas.Sparsity.dense(self.ssm.num_actions,1),cas.Sparsity.dense(self.ssm.num_states,1),cas.Sparsity.dense(self.ssm.num_states,1)][i]
                
            def get_sparsity_out(self,i):
                return cas.Sparsity.dense(2*self.ssm.num_states,self.ssm.num_states+self.ssm.num_actions)
            ...
            def eval(self, arg):
                ...
                mu, sigma, jac_mu, jac_sigma = self.ssm.predict(state,action,True,False)

                jac_pred = np.vstack((jac_mu,jac_sigma))

                return [jac_pred]

        self.jac_callback = JacFun(self.ssm)
    ...
    def get_reverse(self,name,inames,onames,opts):
        raise NotImplementedError("Need to implement this if you set has_reverse = True")

    def get_forward(self,name,inames,onames,opts):
        raise NotImplementedError("Need to implement this if you set has_forward = True")




class DummyModel(StateSpaceModel):
    """ 


    """
    def __init__(self):
        super(DummyModel,self).__init__(2,1)
    ...
    def predict(self, states, actions, jacobians=False, full_cov=False):
        ...
        if jacobians:
            return np.zeros((self.num_states,1)), np.zeros((self.num_states,1)), np.zeros((self.num_states,self.num_states+self.num_actions)), np.zeros((self.num_states,self.num_states+self.num_actions))
        return np.zeros((self.num_states,1)), np.zeros((self.num_states,1))


if __name__ == "__main__":
    model = DummyModel()
    casadi_model = CasadiSSMEvaluator(model)

    m_jac = casadi_model.jacobian_old(1,0)

    #print(np.shape(m_jac(np.array([0,0])[:,None],0)))
    ...
    mu,sigma = casadi_model(x,y)

    x = cas.vertcat(x,y)
    f = cas.sum1(cas.sum2(mu))
    options = {"ipopt": {"hessian_approximation": "limited-memory"}}
    solver = cas.nlpsol("solver","ipopt",{"x":x,"f":f},options)
    res = solver(x0=cas.vertcat(5,1))

    #prob = {'f': gpr(w[0::3]), 'x': x , 'g': vertcat(*g)}
    #options = {"ipopt": {"hessian_approximation": "limited-memory"}}
