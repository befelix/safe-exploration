...
class SimpleGPModel(StateSpaceModel):
    ...
    def __init__(self, n_s_out, n_s_in, n_u, X=None, y=None, m=None, kern_types=None,
        ...
        self.hyp = self._create_hyp_dict(self.base_kerns, self.kern_types)
    ...
    def train(self, X, y, m=None, opt_hyp=True, noise_diag=1e-5, Z=None,
        ...
        for i in range(self.n_s_out):
            ...
            if noise_diag > 0.:
                model_gp.likelihood.variance.fix(model_gp.Gaussian_noise.variance + noise_diag)
    ...
    def _create_hyp_dict(self, kerns, kern_types):
        """ Create a hyperparameter dict from the individual supported kernels

        Parameters
        ----------
        kerns: n_s x 0 array_like[GPy.kern]
            The list of kerns
        kern_type:
        ...
        for i in range(self.n_s_out):
            ...
            if kern_types[i] == "rbf":
                ...
                hyp_i["lengthscale"] = np.reshape(kerns[i].lengthscale, (-1,))
                hyp_i["variance"] = kerns[i].variance
                ...
                hyp_i["prod.rbf.lengthscale"] = np.array(
                    [kerns[i].mul.rbf.lengthscale])
                hyp_i["prod.rbf.variance"] = kerns[i].mul.rbf.variance
                hyp_i["prod.linear.variances"] = np.array(
                    kerns[i].mul.linear.variances)
                hyp_i["linear.variances"] = np.array([kerns[i].linear.variances])

            elif kern_types[i] == "lin_mat52":
                hyp_i["prod.mat52.lengthscale"] = np.array(
                    [kerns[i].mul.Mat52.lengthscale])
                hyp_i["prod.mat52.variance"] = kerns[i].mul.Mat52.variance
                hyp_i["prod.linear.variances"] = np.array(
                    kerns[i].mul.linear.variances)
                hyp_i["linear.variances"] = np.array([kerns[i].linear.variances])
            else:
