...
def run_uncertainty_propagation(env, safempc, conf):
    ...
    for i in range(n_rollouts):
        ...
        if random_actions:
            ...
            _, _, p_traj_i, q_traj_i = multistep_reachability(x_0[:, None],
                                                              k_ff[0, :, None], k_fb,
                                                              k_ff[1:, :], safempc.gp,
                                                              env.l_mu, env.l_sigm,
                                                              c_safety=conf.beta_safety,
                                                              a=a_prior, b=b_prior)
            ...
            for j in range(conf.n_restarts_optimizer):
                ...
                u_0, feasible, _, k_fb, k_ff, p_traj_i, q_traj_i = safempc.solve(
                    x_0[:, None], env.p_origin[:, None], env.p_origin[:, None],
                    sol_verbose=True)
            ...
            if not feasible:
                raise ValueError(
                    "Couldn't find a feasible solution during {} attempts".format(
                        conf.n_restarts_optimizer))

        p_all[i] = p_traj_i
        q_all[i] = q_traj_i
        x_all[i, 0, :] = x_0

        inside_ellipsoid_i = trajectory_inside_ellipsoid(env, x_0, p_traj_i, q_traj_i,
                                                         k_fb, k_ff)
