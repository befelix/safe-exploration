...
def test_matrix_norm_2(before_test_matrix_norm_2):
    ...
    assert np.allclose(f_out, np.max(
        np.linalg.eig(x_in)[0])), "Doesn't return the same as numpy eig"
...
def test_remainder_overapproximation(before_test_remainder_overapproximation):
    ...
    u_mu_cas, u_sigma_cas = utils_cas.compute_remainder_overapproximations(q_cas,
                                                                           k_fb_num,
                                                                           l_mu,
                                                                           l_sigma)
    f = Function("f", [q_cas], [u_mu_cas, u_sigma_cas])
    f_out_cas = f(q_num)
    f_out_py = utils.compute_remainder_overapproximations(q_num, k_fb_num, l_mu,
                                                          l_sigma)

    assert np.allclose(f_out_cas[0],
                       f_out_py[0]), "are the overapproximations of mu the same"
    assert np.allclose(f_out_cas[1],
    ...
    assert np.allclose(f_out_cas_sym_k[0],
                       f_out_py[0]), "are the overapproximations of mu the same"
    assert np.allclose(f_out_cas_sym_k[1],
                       f_out_py[1]), "are the overapproximations of sigma the same"
