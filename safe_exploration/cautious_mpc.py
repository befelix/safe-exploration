...
from casadi import SX, mtimes, vertcat, diag, horzcat, repmat
from casadi import reshape as cas_reshape
from uncertainty_propagation_casadi import multi_step_taylor_symbolic, mean_equivalent_multistep
from utils import dlqr
from gp_reachability_casadi import onestep_reachability, lin_ellipsoid_safety_distance, objective

ATTR_NAMES_ENV = ['h_mat_safe','h_safe','lin_model','ctrl_bounds','h_mat_obs','h_obs' , 'dt']
...
class CautiousMPC:
    ...
    def init_solver(self,cost_func = None):
        ...
        mu_all, sigma_all,sigma_g = self.perf_trajectory(mu_0,self.gp,k_ff,k_fb_all,None,self.a,self.b,self.lin_trafo_gp_input)
            
        self.f_multistep_eval = cas.Function("f_multistep_eval",[mu_0,k_ff,k_fb],[mu_all,sigma_all,sigma_g])

        g, lbg, ubg, g_name = self.generate_safety_constraints(mu_all,sigma_all,k_ff[0,:],k_fb_all,k_ff[1:,:])
